...
class PhpWsdl{
	...
	//var_dump($data);die();
		...
		if (self::$Debugging) {
			$optimizerVal = ($optimizer) ? "YES" : "NO";
			$optimizeVal = ($server->Optimize) ? "YES" : "NO";
			self::Debug("Optimizer: {$optimizerVal}, Optimize: {$optimizeVal}");
			if (!$optimizer&&!$server->Optimize) {
				self::Debug("yes format xml");
			} else {
				self::Debug("yes optimze xml");
			}
		}
